### scp aob2x@rivanna.hpc.virginia.edu:/scratch/aob2x/daphnia_hwe_sims/Rabbit_phase_10cm/all_AxC.rQTL.csv ~/.
...
### load cross
  AxCF1 <- read.cross("csv","",
                      file="/scratch/aob2x/daphnia_hwe_sims/Rabbit_phase_10cm/all_AxC.rQTL.csv",
                      crosstype="4way", genotypes=NULL)

  AxCF1 <- read.cross("csv","",
                      file="~/all_AxC.rQTL.csv",
                      crosstype="4way", genotypes=NULL)
...
### make/fit qtl using peaks from PoolSeq
  ...
  lod <- fitqtl(AxCF1, pheno.col=4, mq,
                formula=paste("y~", paste(paste("Q", c(1), sep=""), collapse="+ "), sep=" "),
                method="imp")
  summary(lod)
  rqtl <- refineqtl(AxCF1, qtl=mq, method="imp")
