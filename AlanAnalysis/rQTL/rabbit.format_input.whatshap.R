...
### pull in phased parents from whatshapp
  phased <- fread("/scratch/aob2x/daphnia_hwe_sims/trioPhase/rQTL_quartet.consensus.header.phase.noNA.vcf",
                  skip="#CHROM")
  setnames(phased, c("#CHROM", "POS"), c("chr", "pos"))
  setkey(phased, "chr")

  phased <- phased[J(chr.i)]

  setkey(phased, chr, pos)
  phased <- merge(phased, snp.dt)

### format parents
  formatParent <- function(x) {
    x <- gsub("1", "2", x)
    x <- gsub("0", "1", x)
    x <- gsub("\\|", "", x)
    x <- gsub("/", "", x)
  ...
  A.parent <- c("A", formatParent(tstrsplit(phased$A, ":")[[1]]))
  C.parent <- c("C", formatParent(tstrsplit(phased$C, ":")[[1]]))
