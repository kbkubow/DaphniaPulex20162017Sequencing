...
### figure out who is root
  ...
  #use.root <- sc.root[Species%in%c("Obtusa")]$clone
  #use.root <- sc.root[SC%in%c("A")]$clone
...
### run windows
  m <- foreach(win.i = c(1:dim(wins)[1])[1:10], .errorhandling="remove")%dopar%{
...
### plot
  ...
  m.new <- foreach(m.i=1:length(m))%do%{
    ...
    tmp <- root(tmp, clones[clone=="Spring_2016_W1_1.1"]$lab)
    tmp
  }
