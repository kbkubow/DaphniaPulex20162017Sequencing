...
### run windows
  ...
  #Now let's aggregate the file comparing everyone except A and C to obtusa and pulex
      mscothervsObtusaPulicaria.ag <- mscothervsObtusaPulicaria[,list(meanIBS=mean(IBS), sd_IBS=sd(IBS)),
  ...
  #  mscACvsrest_AW.ag <- mscACvsrest.ag
    ...
    save(mscothervsObtusaPulicaria.ag, file=paste("mscothervsObtusaPulicaria_", varA, ".Rdata", sep=""))

  ### Load processed IBS by window files
      inputfiles <- list.files(path="/scratch/kbb7sh/Daphnia/MappingDecember2019/WithPulicaria/", pattern="mscACvsrest.ag_2020041")
      ...
      inputfilesPO <- list.files(path="/scratch/kbb7sh/Daphnia/MappingDecember2019/WithPulicaria/", pattern="mscACvsrest_POBC_20200417")
    ...
    ggplot(data=mscACvsrest_AW.ag, aes(x=SCcompare, y=meanIBS, group=SCcompare, color=ACvspond)) + geom_boxplot()
