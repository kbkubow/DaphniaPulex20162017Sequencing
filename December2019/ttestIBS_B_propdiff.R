#!/usr/bin/env Rscript
  ...
  AvsC_A_D10 <- foreach(i=1:length(A_D10SCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_A_D10", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_A_W1 <- foreach(i=1:length(A_W1SCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_A_W1", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_A_W6 <- foreach(i=1:length(A_W6SCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_A_W6", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_A_pulicaria <- foreach(i=1:length(A_pulicariaSCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_A_pulicaria", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_A_obtusa <- foreach(i=1:length(A_obtusaSCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_A_obtusa", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_C_D10 <- foreach(i=1:length(C_D10SCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_C_D10", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_C_W1 <- foreach(i=1:length(C_W1SCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_C_W1", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_C_W6 <- foreach(i=1:length(C_W6SCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_C_W6", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_C_pulicaria <- foreach(i=1:length(C_pulicariaSCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_C_pulicaria", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }
  ...
  AvsC_C_obtusa <- foreach(i=1:length(C_obtusaSCcomp), .combine="rbind")%do%{
    ...
    m <- merge(AvsCsub, tmpsub)
    m$ratiodiff <- m$AvsCmeanIBS/m$tmp
    stats <- data.table(comppond="AvsC_C_obtusa", compsc=sc, mean_ratiosim=mean(m$ratiodiff),
      sd_ratiosim=sd(m$ratiodiff))
  }

  totalpropsimACACoutgroup <- rbind(AvsC_A_D10, AvsC_A_W1, AvsC_A_W6, AvsC_A_pulicaria, AvsC_A_obtusa,
    AvsC_C_D10, AvsC_C_W1, AvsC_C_W6, AvsC_C_pulicaria, AvsC_C_obtusa)

  save(totalpropsimACACoutgroup, file="totalpropsimACACoutgroup_20200818.Rdata")
  ...
  load("totalttestACotherpaired_20200817.Rdata")
  load("totalttestACACoutgrouppaired_20200817.Rdata")
  load("totalDBunkD8DCatvsOutpaired_20200817.Rdata")

  totalttest <- rbind(totalttestACotherpaired, totalttestACACoutgrouppaired, totalDBunkD8DCatvsOutpaired)

  totalttest$comppond <- factor(totalttest$comppond, levels=c("AvsC_D8D8", "AvsC_DBunkDBunk", "AvsC_DCatDCat",
    "AvsC_D8DBunk", "AvsC_D8DCat", "AvsC_DBunkDCat", "AvsC_A_D10", "AvsC_A_W1", "AvsC_A_W6",
    "AvsC_A_pulicaria", "AvsC_A_obtusa", "AvsC_C_D10", "AvsC_C_W1", "AvsC_C_W6", "AvsC_C_pulicaria",
    "AvsC_C_obtusa", "AvsC_D8DBunkDCatvsD10", "AvsC_D8DBunkDCatvsW6", "AvsC_D8DBunkDCatvsPulicaria",
    "AvsC_D8DBunkDCatvsObtusa"))

  ggplot(data=totalttest, aes(x=comppond, y=t, group=comppond)) + geom_boxplot() +
  ...
  ggplot(data=totalttest, aes(x=comppond, y=t, group=comppond)) + geom_violin() +
  geom_hline(yintercept=0,color="red") +
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
