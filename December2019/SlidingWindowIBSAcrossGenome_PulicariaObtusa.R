...
### Read in only G files first to figure out how to make the figure
      ...
      ObtusaPulicaria <- total[type=="vsObtusa" | type=="vsPulicaria"]
      window <- unique(ObtusaPulicaria$window)
      ACvspondu <- unique(ObtusaPulicaria$ACvspondB)

      focalbywindowbrokendownObPul <- foreach(n=1:length(ACvspondu), .combine="rbind")%do%{
          AC <- ACvspondu[n]
          subsetfocal <- ObtusaPulicaria[ACvspondB==AC]
      ...
      save(focalbywindowbrokendownObPul, file="focalbywindowbrokendownObPul_20201016.Rdata")
