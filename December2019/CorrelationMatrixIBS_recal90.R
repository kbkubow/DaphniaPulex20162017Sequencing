...
#Load SNP file
  load("finalsetsnpset01_20200722.Rdata")
		...
		ibs.long<- melt(ibs.matdt, measure.vars=scsubids, variable.name="cloneB", value.name="IBS")
  		...
  		### be lazy and write a loop
  					...
  					### group on pond
  					    ...
  					    ibs.long[,pondA := factor(pondA, levels=c("D10", "D8", "DBunk", "DCat"))]
  					    ibs.long[,pondB := factor(pondB, levels=c("D10", "D8", "DBunk", "DCat"))]
                    ...
                    ggsave(corrmatrix, file="corrmatrix_20200706.pdf")

                      corrmatrixwithbox <- ggplot(data=ibs.long, aes(scid.a, scid.b, fill=IBS)) +
                        geom_raster() +
                        scale_fill_viridis(option="D") +
